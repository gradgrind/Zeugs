...
def Manager(schoolyear, gclass, gstream, grademap, trap_missing = True):
#    if gclass >= '13':
#TODO: Handle this (Abitur results) separately:
#        if term == 'A':
#            return GradeManagerA
#        return GradeManagerQ1
    if gclass >= '12' and gstream == 'Gym':
        return GradeManagerQ1(schoolyear, gclass, gstream, grademap,
                trap_missing)
    return GradeManagerN(schoolyear, gclass, gstream, grademap,
                trap_missing)
        ...
        for sid, sdata in gsubjects.items():
            if sdata.tids:
                ...
                try:
                    ...
                    if trap_missing:
                        raise GradeError(_GRADE_MISSING.format(
