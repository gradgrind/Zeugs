...
def toPdf (folder, *files, pdf='pdf'):
    ...
    subfolder pdf, by default 'pdf'.
    """
# One call possibility:
#   libreoffice --headless --convert-to pdf --outdir pdf <files>
# Here the --outdir parameter is not used, but the working directory is changed
# before calling libreoffice.
...
#TODO: Message
    ...
    rc, msg = run_extern ('EXEC_LO', '--headless', '--convert-to' ,'pdf',
            '--outdir', pdf, *files,
            cwd=folder, capture_output=True, feedback=True)
    if rc == 0:
...
def toPrinter (pdffile, folder=None):
    ...
    try:
        ...
        cmdx = ''
    try:
        cmd = list (CONFIG.APPS.get (command + '_' + cmdx)
                or CONFIG.APPS.get (command))
    except:
